...
describe('MyOFT Test', function () {
    ...
    it('deploys the production OFT with zero initial supply and no public mint function', async function () {
        const ProductionOFT = await ethers.getContractFactory('StonksOFT')
        const production = await ProductionOFT.deploy('Test STONKS', 'tSTONKS', mockEndpointV2A.address, ownerA.address)

        expect(await production.name()).to.equal('Test STONKS')
        expect(await production.symbol()).to.equal('tSTONKS')
        expect(await production.totalSupply()).to.equal(0)
        expect(await production.owner()).to.equal(ownerA.address)
        expect(production.interface.functions['mint(address,uint256)']).to.equal(undefined)
    })
})
